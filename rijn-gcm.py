...
def aes_encrypt(plaintext, key, iv, ad):
    ...
    C = gctr(J_0, plaintext, key)
    T = bytes(a ^ b for a, b in zip(ghash(H, ad, C), ecb_encrypt(J_0, key)))
    
    return C, T

def aes_decrypt(ciphertext, tag, key, iv, ad):
    H = ecb_encrypt(b"\x00" * 16, key)
    if len(iv) == 12:
        J_0 = iv + b"\x00\x00\x00\x01"
    else:
    ...
    if decrypted_tag != tag:
        print("Authentication failed: tags do not match, continue with caution.")
...
def encode():
    ...
    additional_data = b''
    # Note: Make sure you save the tag, as you need 
    # it for decryption, but not for encryption
    
    ciphertext, tag = aes_encrypt(plaintext, key, iv, additional_data)
    decrypted_plaintext = aes_decrypt(ciphertext, tag, key, iv, additional_data)

""" Variables """

S_BOX = [
    
] # Byte Substitution Box

INV_S_BOX = [
    
] # Inverse Byte Substitution Box

RCON = (
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a, 0x2f
)

base64_alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'

# KEY EXPANSION WORKING WITH TEST: https://www.kavaliro.com/wp-content/uploads/2014/03/AES.pdf (PAGE 3)
"""
def key_expansion(key):
    key = key.encode('utf-8')
    key_size = len(key)
    key_words = [key[i:i + 4] for i in range(0, len(key), 4)]
    expanded_keys = list(key_words)
    
    if key_size == 16:
        Nr = 10  # 10 rounds for 128-bit key
    elif key_size == 24:
        Nr = 12  # 12 rounds for 192-bit key
    elif key_size == 32:
        Nr = 14  # 14 rounds for 256-bit key

    Nk = key_size // 4

    for i in range(Nk, 4 * (Nr + 1)):
        temp = expanded_keys[i - 1]

        if key_size == 32 and i % Nk == 4:
            # Additional logic for 256-bit key expansion
            temp = [S_BOX[b] for b in temp]

        elif i % Nk == 0:
            # RotWord
            temp = [temp[1], temp[2], temp[3], temp[0]]

            # SubWord
            for j in range(4):
                temp[j] = S_BOX[temp[j]]

            # XOR with Rcon
            temp[0] ^= RCON[i // Nk - 1]

        # XOR with previous word
        expanded_keys.append([a ^ b for a, b in zip(expanded_keys[i - Nk], temp)])

    return expanded_keys

# The below function is only for testing.
def print_matrix(m):
    num_keys = len(m)
    for round_num in range(num_keys):
        if round_num % 4 == 0:
            print('\nRound {:2}:'.format(round_num // 4), end=' ')
        print('{}'.format(' '.join("{:02X}".format(x) for x in m[round_num])), end=' ')
    print(" ")

# Example key
key = 'Thats my Kung FuThats my Kung Fu' 
print(f"key: {key}")

# Perform key expansion
expanded_keys = key_expansion(key)

# Display the expanded keys
print("Expanded Keys:")
print_matrix(expanded_keys)
"""
# SUB_BYTES
"""
def sub_bytes(state):
    for i in range(len(state)):
        for j in range(len(state[i])):
            # Substitute the byte with the S-box value
            state[i][j] = S_BOX[state[i][j]]
    
    return state

def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            # Substitute the byte with the inverted S-box value
            state[i][j] = INV_S_BOX[state[i][j]]

    return state

state = [[2, 7, 17, 29], [41, 53, 67, 79], [97, 107, 127, 139], [157, 173, 191, 199]]

print(f"State before: {state}")

state = sub_bytes(state)

print(f"State after sub: {state}")

state = inv_sub_bytes(state)

print(f"State after inv: {state}")
"""
# SHIFT_ROWS
# THIS HAS CHANGED IN LATEST VERSION (CHECK LINE 322)
"""
def shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]
    return state

def inv_shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][-i:] + state[i][:-i]
    return state

state = [[2, 7, 17, 29], [41, 53, 67, 79], [97, 107, 127, 139], [157, 173, 191, 199]]

print(f"State before funct: {state}")

state = shift_rows(state)

print(f"State after shift: {state}")

state = inv_shift_rows(state)

print(f"State after inver: {state}")
"""
# MIX COLUMNS
"""
def mix_columns(state):
    for i in range(4):
        s = [state[i][j] for j in range(4)]

        state[i][0] = mul(0x02, s[0]) ^ mul(0x03, s[1]) ^ s[2] ^ s[3]
        state[i][1] = s[0] ^ mul(0x02, s[1]) ^ mul(0x03, s[2]) ^ s[3]
        state[i][2] = s[0] ^ s[1] ^ mul(0x02, s[2]) ^ mul(0x03, s[3])
        state[i][3] = mul(0x03, s[0]) ^ s[1] ^ s[2] ^ mul(0x02, s[3])

    return state

def inv_mix_columns(state):
    # The inverse matrix for MixColumns
    inv_matrix = [
        [0x0e, 0x0b, 0x0d, 0x09],
        [0x09, 0x0e, 0x0b, 0x0d],
        [0x0d, 0x09, 0x0e, 0x0b],
        [0x0b, 0x0d, 0x09, 0x0e]
    ]

    for i in range(4):
        s0 = state[i][0]
        s1 = state[i][1]
        s2 = state[i][2]
        s3 = state[i][3]

        state[i][0] = (
            (mul(0x0e, s0) ^ mul(0x0b, s1) ^ mul(0x0d, s2) ^ mul(0x09, s3))
        ) & 0xFF
        state[i][1] = (
            (mul(0x09, s0) ^ mul(0x0e, s1) ^ mul(0x0b, s2) ^ mul(0x0d, s3))
        ) & 0xFF
        state[i][2] = (
            (mul(0x0d, s0) ^ mul(0x09, s1) ^ mul(0x0e, s2) ^ mul(0x0b, s3))
        ) & 0xFF
        state[i][3] = (
            (mul(0x0b, s0) ^ mul(0x0d, s1) ^ mul(0x09, s2) ^ mul(0x0e, s3))
        ) & 0xFF

    return state

def mul(a, b):
    # Multiplication in GF(2^8) finite field
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        a &= 0xFF  # Keep only 8 bits
        if hi_bit_set:
            a ^= 0x1b  # XOR with irreducible polynomial x^8 + x^4 + x^3 + x + 1
        b >>= 1
    return p

state = [[219, 19, 83, 69], [242, 10, 34, 92], [212, 212, 212, 213], [45, 38, 49, 76]]

print(f"State before: {state}")

state = mix_columns(state)

print(f"State after mix: {state}")

state = inv_mix_columns(state)

print(f"State after inv: {state}")
"""
# ADD ROUND KEY
"""
def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]

    return state

def inv_add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]

    return state

state = [[219, 19, 83, 69], [242, 10, 34, 92], [212, 212, 212, 213], [45, 38, 49, 76]]

key = [
    [123, 234, 1, 12],
    [45, 67, 89, 200],
    [201, 190, 155, 100],
    [255, 128, 64, 32]
]

print(f"key: {key}")
print(f"State before: {state}")

state = add_round_key(state, key)

print(f"State after ARK: {state}")

state = inv_add_round_key(state, key)

print(f"State after inv: {state}")
"""
# Base64
"""
import base64

def base64_encode_bytes(byte_string):
    output_text = ""
    bits = 0
    bits_count = 0

    for byte in byte_string:
        bits <<= 8
        bits += byte
        bits_count += 8

        while bits_count >= 6:
            index = (bits >> (bits_count - 6)) & 63
            output_text += base64_alphabet[index]
            bits_count -= 6
            bits &= (1 << bits_count) - 1

    if bits_count > 0:
        bits <<= 6 - bits_count
        index = bits & 63
        output_text += base64_alphabet[index]

    # Add padding to make the length a multiple of 4
    while len(output_text) % 4 != 0:
        output_text += "="

    return output_text

def base64_decode_text(encoded_text):
    byte_string = bytearray()
    bits = 0
    bits_count = 0

    # Decoding base64 characters back to bytes
    for char in encoded_text:
        if char == '=':
            break  # Padding indicates the end of the encoded text

        index = base64_alphabet.index(char)  # Get index of the character in the base64 alphabet
        bits = (bits << 6) + index
        bits_count += 6

        while bits_count >= 8:
            bits_count -= 8
            decoded_byte = (bits >> bits_count) & 0xFF
            byte_string.append(decoded_byte)

    return bytes(byte_string)

my_bytes = b"Hello World! !@#$%^&*()_+"
print(f"ogl string bytes: {my_bytes}")
print(f"base64.b64encode: {base64.b64encode(my_bytes)}")
print(f"base64 encode fn: {base64_encode_bytes(my_bytes)}")
print(f"base64 decode fn: {base64_decode_text(base64_encode_bytes(my_bytes))}")
print(f"base64.b64decode: {base64.b64decode(base64.b64encode(my_bytes))}")
"""
# Shift Columns (shift rows update)
# No tests available, comfirmed in main file (AES.py)
"""
def shift_rows(state):
    # Shifting each column in the state matrix
    for i in range(4):  # For each column
        state[0][i], state[1][i], state[2][i], state[3][i] = (
            state[(0 + i) % 4][i],
            state[(1 + i) % 4][i],
            state[(2 + i) % 4][i],
            state[(3 + i) % 4][i],
        )
    return state

def inv_shift_rows(state):
    # Shifting each column in the state matrix (inverse operation)
    for i in range(4):  # For each column
        state[0][i], state[1][i], state[2][i], state[3][i] = (
            state[(0 - i) % 4][i],
            state[(1 - i) % 4][i],
            state[(2 - i) % 4][i],
            state[(3 - i) % 4][i],
        )
    return state
"""